...
def chunk_audio_and_run(audio_file, chunk_size = 500000, device="cpu"):
    ...
    chunks = torch.split(audio_feats, chunk_size, dim=1)
    preds_chunks = []
    for i, chunk in enumerate(chunks):
        _, preds = run_acoustic_based(chunk, model)
        preds_chunks.append(preds)
    
    preds_chunks = np.vstack(preds_chunks)
    return preds_chunks

def run_acoustic_based(audio_feats, model, device="cpu"):
...
def calculate_confidence_score_acoustic(audio_path, multi_factor=7, save_result_df= False):
    preds = chunk_audio_and_run(audio_path)
    pred_df = pd.DataFrame(preds, columns=labels).astype(int)
    ...
    return result  
print(calculate_confidence_score_acoustic("jaishankar.mp3"))